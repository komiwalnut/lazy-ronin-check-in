...
const CHECKIN_FUNCTION_PREFIX = "0xd9a59e33";
const MAX_RETRIES = 3;
...
export async function sendCheckInTransaction(): Promise<string> {
  let attemptCount = 0;
  ...
  while (attemptCount < MAX_RETRIES) {
    for (const rpc of endpointsToTry) {
      try {
        ...
        const callData = encodeCheckInData(wallet.address);
        ...
        const gasLimit = await estimateGas(wallet.address, rpc.url, callData);
        ...
        logger.info(`Preparing to call checkIn() on contract: ${config.contractAddress}`);
        ...
        try {
          ...
          if (errorMsg.includes("execution reverted")) {
            ...
            logger.warn("The contract may have time restrictions or other conditions for the checkIn() function.");
          } else {
